...
bool IsNewsBlocked()
{
    ...
    if(NewsTimes != gNewsTimesCached)
    {
        ...
        for(int i=0;i<n;i++)
        {
            ...
            ArrayResize(gNewsTimes, ArraySize(gNewsTimes)+1);
            gNewsTimes[ArraySize(gNewsTimes)-1] = dt;
        }
    }
...
{
    double lowVal = iLow(_Symbol, PERIOD_CURRENT, 0);
    for(int i=1; i<=lookback; i++)
    {
        double l = iLow(_Symbol, PERIOD_CURRENT, i);
...
bool IsPinBar(int shift=1)
{
    ...
    return (body < (high-low)*0.3 && lowerShadow > body*2);
}
...
bool IsHammer(int shift=1)
{
    ...
    double lowerShadow = MathMin(open, close) - low;
    double upperShadow = high - MathMax(open, close);
    return (body < (high-low)*0.3 && lowerShadow > body*2 && upperShadow < body);
}
...
bool IsShootingStar(int shift=1)
{
    ...
    double upperShadow = high - MathMax(open, close);
    double lowerShadow = MathMin(open, close) - low;
    return (body < (high-low)*0.3 && upperShadow > body*2 && lowerShadow < body);
}
...
void UpdateDashboard(string signal)
{
   ...
   // ========== Balance Box ==========
   if(ObjectFind(0,UM_Box_Bal) < 0)
      ObjectCreate(0,UM_Box_Bal,OBJ_RECTANGLE_LABEL,0,0,0);
   ObjectSetInteger(0,UM_Box_Bal,OBJPROP_CORNER,    CORNER_LEFT_UPPER);
   ObjectSetInteger(0,UM_Box_Bal,OBJPROP_XDISTANCE, x);
   ObjectSetInteger(0,UM_Box_Bal,OBJPROP_YDISTANCE, y1);
   ObjectSetInteger(0,UM_Box_Bal,OBJPROP_XSIZE,     boxW);
   ObjectSetInteger(0,UM_Box_Bal,OBJPROP_YSIZE,     boxH);
   ObjectSetInteger(0,UM_Box_Bal,OBJPROP_BGCOLOR,   DashBGColor);
   ObjectSetInteger(0,UM_Box_Bal,OBJPROP_COLOR,     DashBGColor);
   
   string balTxt = "Balance: " + DoubleToString(balance,2);
   if(ObjectFind(0,UM_Box_Bal_Txt) < 0)
      ObjectCreate(0,UM_Box_Bal_Txt,OBJ_LABEL,0,0,0);
   ObjectSetInteger(0,UM_Box_Bal_Txt,OBJPROP_CORNER,    CORNER_LEFT_UPPER);
   ObjectSetInteger(0,UM_Box_Bal_Txt,OBJPROP_XDISTANCE, x+10);
   ObjectSetInteger(0,UM_Box_Bal_Txt,OBJPROP_YDISTANCE, y1+8);
   ObjectSetInteger(0,UM_Box_Bal_Txt,OBJPROP_COLOR,     DashFontColor);
   ObjectSetInteger(0,UM_Box_Bal_Txt,OBJPROP_FONTSIZE,  DashFontSize);
   ObjectSetString (0,UM_Box_Bal_Txt,OBJPROP_TEXT,      balTxt);

   // ========== Profit Box ==========
   if(ObjectFind(0,UM_Box_Pft) < 0)
      ObjectCreate(0,UM_Box_Pft,OBJ_RECTANGLE_LABEL,0,0,0);
   ObjectSetInteger(0,UM_Box_Pft,OBJPROP_CORNER,    CORNER_LEFT_UPPER);
   ObjectSetInteger(0,UM_Box_Pft,OBJPROP_XDISTANCE, x);
   ObjectSetInteger(0,UM_Box_Pft,OBJPROP_YDISTANCE, y2);
   ObjectSetInteger(0,UM_Box_Pft,OBJPROP_XSIZE,     boxW);
   ObjectSetInteger(0,UM_Box_Pft,OBJPROP_YSIZE,     boxH);
   ObjectSetInteger(0,UM_Box_Pft,OBJPROP_BGCOLOR,   DashBGColor);
   ObjectSetInteger(0,UM_Box_Pft,OBJPROP_COLOR,     DashBGColor);
   
   string pftTxt = "Floating P/L: " + DoubleToString(floating,2) + 
                   "   |  Today: " + DoubleToString(todayProfit,2) +
                   "   |  Signal: " + signal;
   if(ObjectFind(0,UM_Box_Pft_Txt) < 0)
      ObjectCreate(0,UM_Box_Pft_Txt,OBJ_LABEL,0,0,0);
   ObjectSetInteger(0,UM_Box_Pft_Txt,OBJPROP_CORNER,    CORNER_LEFT_UPPER);
   ObjectSetInteger(0,UM_Box_Pft_Txt,OBJPROP_XDISTANCE, x+10);
   ObjectSetInteger(0,UM_Box_Pft_Txt,OBJPROP_YDISTANCE, y2+8);
   ObjectSetInteger(0,UM_Box_Pft_Txt,OBJPROP_COLOR,     DashFontColor);
   ObjectSetInteger(0,UM_Box_Pft_Txt,OBJPROP_FONTSIZE,  DashFontSize);
   ObjectSetString (0,UM_Box_Pft_Txt,OBJPROP_TEXT,      pftTxt);

   // ========== Drawdown Box ==========
   if(ObjectFind(0,UM_Box_DD) < 0)
      ObjectCreate(0,UM_Box_DD,OBJ_RECTANGLE_LABEL,0,0,0);
   ObjectSetInteger(0,UM_Box_DD,OBJPROP_CORNER,    CORNER_LEFT_UPPER);
   ObjectSetInteger(0,UM_Box_DD,OBJPROP_XDISTANCE, x);
   ObjectSetInteger(0,UM_Box_DD,OBJPROP_YDISTANCE, y3);
   ObjectSetInteger(0,UM_Box_DD,OBJPROP_XSIZE,     boxW);
   ObjectSetInteger(0,UM_Box_DD,OBJPROP_YSIZE,     boxH);
   ObjectSetInteger(0,UM_Box_DD,OBJPROP_BGCOLOR,   DashBGColor);
   ObjectSetInteger(0,UM_Box_DD,OBJPROP_COLOR,     DashBGColor);
   
   string ddTxt  = "DD: " + DoubleToString(dd_abs,2) + 
                   " (" + DoubleToString(dd_pct,2) + "%)";
   if(ObjectFind(0,UM_Box_DD_Txt) < 0)
      ObjectCreate(0,UM_Box_DD_Txt,OBJ_LABEL,0,0,0);
   ObjectSetInteger(0,UM_Box_DD_Txt,OBJPROP_CORNER,    CORNER_LEFT_UPPER);
   ObjectSetInteger(0,UM_Box_DD_Txt,OBJPROP_XDISTANCE, x+10);
   ObjectSetInteger(0,UM_Box_DD_Txt,OBJPROP_YDISTANCE, y3+8);
   ObjectSetInteger(0,UM_Box_DD_Txt,OBJPROP_COLOR,     DashFontColor);
   ObjectSetInteger(0,UM_Box_DD_Txt,OBJPROP_FONTSIZE,  DashFontSize);
   ObjectSetString (0,UM_Box_DD_Txt,OBJPROP_TEXT,      ddTxt);
}
